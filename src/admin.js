...
const orderCard = order => {
  ...
  return `<article class="admin-order status-${order.status}">
    ...
    ${order.discount ? `<div class="admin-note">Cupom ${escapeHtml(order.couponCode)}: -${money(order.discount)}</div>` : ''}
...
const printArea = order => {
  ...
  return `<section id="thermal-print">${printLogo()}<div class="print-kitchen-label">COMANDA DE PEDIDO</div><div class="print-order"><span>PEDIDO</span><b>#${escapeHtml(order.number)}</b><time>${formatDateTime(order.createdAt)}</time></div><div class="print-customer"><b>CLIENTE</b><span>${escapeHtml(order.customer.name)}</span><span>${escapeHtml(order.customer.phone)}</span></div><div class="print-section-title">ITENS DO PEDIDO</div><ul>${orderItems(order)}</ul>${order.orderNote ? `<p class="print-note"><b>ATENÇÃO, OBSERVAÇÃO</b><span>${escapeHtml(order.orderNote)}</span></p>` : ''}<div class="print-totals"><span>Subtotal <b>${money(order.subtotal)}</b></span>${order.discount ? `<span>Cupom ${escapeHtml(order.couponCode)} <b>-${money(order.discount)}</b></span>` : ''}${order.deliveryFee ? `<span>Entrega <b>${money(order.deliveryFee)}</b></span>` : ''}<strong><span>TOTAL</span><b>${money(order.total)}</b></strong></div><div class="print-delivery"><b>${order.fulfillment === 'delivery' ? 'ENTREGA' : 'RETIRADA'}</b>${deliveryAddress}</div><div class="print-payment"><b>PAGAMENTO: ${paymentLabels[order.payment.method].toUpperCase()}</b>${order.payment.method === 'cash' ? `<span>Receber: ${order.payment.noChange ? 'valor exato' : money(order.payment.cashAmount)}</span><span>Troco: ${money(order.payment.change)}</span>` : ''}${['credit','debit'].includes(order.payment.method) ? '<strong>LEVAR MAQUININHA</strong>' : ''}${order.payment.method === 'pix' ? '<strong>CONFIRMAR COMPROVANTE</strong>' : ''}</div><footer>Conferido por: __________________</footer></section>`
}
...
const buildRawBtReceipt = order => {
  ...
  if (order.discount) { line(`Cupom ${rawText(order.couponCode)}`); pair('Desconto 10%', `-${rawMoney(order.discount)}`) }
